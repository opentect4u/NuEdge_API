...
<table cellpadding="0" cellspacing="0" width="700" align="center"
   ...
   <tbody>
                           ...
                           <tbody>
                              <tr>
                                 <td align="left" height="80">
                                    <img src="{{ asset('public/site-logo/logo.png') }}" height="80" alt="Nuedge Logo"
                           ...
                           <tbody>
                              ...
                              <tr style="background:#f6f6f6">
                                 ...
                                 <td
                                    ...
                                    $isExpired =($expected_close_date < $actual_close_date)?'Yes':'No'; // return
                                       $isExpired; }else{ if($data->expected_close_date){
                                       $expected_close_date = strtotime($data->expected_close_date);
                                       $today=strtotime(date('Y-m-d'));
                                       $isExpired =($expected_close_date < $today)?'Yes':'No'; } else{
                                          $cal_date=date('Y-m-d', strtotime($data->date_time. ' + '.$data->query_tat.'
                                          day'));
                                          $new_date = \Carbon\Carbon::create($cal_date);
                                          $isAfter = \Carbon\Carbon::create(date('Y-m-d'))->diffInDays($new_date);
                                          $isExpired = ($isAfter <= 0) ?'Yes':'No'; } } echo $isExpired; } @endphp </td>
                              </tr>
                              <tr>
                                 <td valign="top"
                                    style="width:20%;font-weight:600;color:#000;font-size:14px;font-family:'calibri','Segoe UI','Helvetica Neue',Helvetica,Arial,sans-serif;padding:6px 0 6px 10px;border-right:1px solid #ddd;border-bottom:1px solid #ddd;border-left:1px solid #ddd">
                              ...
                              <tr style="background:#f6f6f6">
                                 ...
                                 <td colspan="3"
                                    ...
                                    <a href="{{env('QUERY_DOWNLOAD_LINK').Crypt::encrypt($attachment->id)}}">Click Here
                                       to download attachment {{($key+1)}}</a>
                           ...
                           <tbody>
                              <tr>
                                ...
                                border-right:1px solid #ddd;border-bottom:1px solid #ddd;border-left:1px solid #ddd;border-top:1px solid #ddd">
                                    ...
                                    <a href="{{$data->feedback_url}}">
                                       <img src="{{asset('public/email-logo/feedback.gif')}}" alt="" srcset=""
                                       width="155px" height="36px" style="object-fit: none;">
                                    </a>
    ...
    filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#ffc107', endColorstr='#ff5722', GradientType=1);
            ...
            <table style="padding:0 10px;width:100%;border:solid 1px #ccc" cellspacing="0 " cellpadding="0 " border="0 "
