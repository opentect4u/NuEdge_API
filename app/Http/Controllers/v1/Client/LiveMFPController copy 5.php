...
class LiveMFPController extends Controller
{
                ...
                if ($data_key==10) {
                    ...
                    foreach ($redemption_data as $key => $redemption_value) {
                        ...
                        foreach ($purchase_data as $purchase_key => $purchase_value) {
                            // if ($key==6) {
                            //     // return $rdm_tot_units." -- ".$purchase_value['cumml_units'];
                            //     return $purchase_data;
                            //     return $deduct_unit_array;
                            ...
                            // $purchase_value['cumml_units']= ((float)$purchase_cumml_units - (float)$rdm_tot_units);
                            $purchase_cumml_val=(number_format((float)$purchase_cumml_units, 4, '.', '') - number_format((float)$rdm_tot_units, 4, '.', ''));
                            $purchase_value['cumml_units']=$purchase_cumml_val;

                            // if ($key==6) {
                            //     return $purchase_value['cumml_units'];
                            //     return (number_format((float)$purchase_cumml_units, 4, '.', '') - number_format((float)$rdm_tot_units, 4, '.', ''));
                            //     return $deduct_unit_array;
                            // }
                            if ($purchase_value['cumml_units'] >= 0) {
                                ...
                                if ($deduct_unit_array[(count($deduct_unit_array)-1)]['cumml_units'] <=  0) {
                                    ...
                                    //     // return $purchase_data;
                                    // }
                                    // if ((float)$deduct_unit_array[(count($deduct_unit_array)-1)]['cumml_units'] <=  0) {
                                    ...
                                    // return $purchase_value['cumml_units'];
                                    // if ($key==7) {
                                        if (count($after_remaining_array)> 0) {
                                        ...
                                        // return $purchase_data;
                                    // }
                                    // $cal_aaaa=(count($deduct_unit_array))?(float)$deduct_unit_array[(count($deduct_unit_array)-2)]['cumml_units']:0;
                                    // if ($cal_aaaa <= 0 && $purchase_value['cumml_units'] > 0) {
                                    //     $set_units=$purchase_value['cumml_units'];
                                    //     $purchase_value['cumml_units']=$set_units;
                                    //     $deduct_unit_array=[...$deduct_unit_array,$purchase_value];
                                    //     $rdm_tot_units=0;
    ...
    // public function calculate($mydata){
