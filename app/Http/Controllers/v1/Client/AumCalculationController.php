...
class AumCalculationController extends Controller
{
    ...
    public function test()
    {
        ...
        foreach ($clients as $key => $client) {
            $port=AumCalculationController1::__calucationTotUnitsAndInvCost($client->first_client_name,$client->first_client_pan,$valuation_as_on);
            return $port;
        }
        // AumCalculationJob::dispatch($clients);
    ...
    public static function __calucationTotUnitsAndInvCost($client_name,$pan_no,$valuation_as_on)
    {
        try {
            ...
            foreach ($all_data as $data_key => $value1) {
                ...
                // if ($value1['product_code']=="128MLGP") {
                //     return $foliotrans;
                // }
                ...
                $mydata=AumCalculationController1::__calculate($foliotrans,$value1['curr_nav'],$valuation_as_on);
                // return $mydata;
                // if ($value1['product_code']=="128MLGP") {
                //     $mydata=AumCalculationController1::__calculate($foliotrans,$value1['curr_nav'],$valuation_as_on);
                //     return $mydata;
    ...
    public static function __calculate($foliotrans,$curr_nav,$valuation_as_on){
        $purchase_data=[];
        $redemption_data=[];
        $purchase_amt_arr=[];
        $redemption_amt_arr=[];
