...
class PDFController extends Controller
{
    ...
    public function autoDownloadValuation(Request $request){
        $token=$request->token;
        $is_has=DB::table('td_portfolio_valuation_details')->where('token',$token)->get();
        if (count($is_has) > 0) {
            $headers = ['Content-Type'  => 'application/pdf','Content-Disposition' =>'attachment; filename=valuation.pdf'];
            return Response::make(file_get_contents($is_has[0]->url), 200,$headers);
        }else {
