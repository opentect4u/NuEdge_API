    ...
    public function search(Request $request)
    {
        try {
            ...
            $all_data=MutualFundTransaction::leftJoin('md_scheme_isin','md_scheme_isin.product_code','=','td_mutual_fund_trans.product_code')
                ...
                ->get();
            dd(DB::getQueryLog());
                // SELECT  scheme_name,cat_name,product_code 
                ...
                // WHERE first_client_pan='DIUPM6199D'
                // GROUP BY scheme_name,cat_name,product_code,
                // subcat_name,amc_name,plan_name,option_name,isin_no,nifty50,sensex
                // ORDER BY scheme_name ASC LIMIT 5
            ...
            // return $all_data;
                ...
                foreach ($all_data as $key => $value) {
                    ...
                    $value->inv_since=MutualFundTransaction::where('folio_no',$value->folio_no)
                        ...
                        ->first();
                    // return $value->inv_since->trans_date;
                    // $response = Http::get('https://sia72vzf17.execute-api.ap-south-1.amazonaws.com/dev/showData?product_code='.$value->product_code.'&nav_date='.date('Y-m-d',strtotime($value->inv_since->trans_date)));
                    // // return $response;
                    // $value->curr_nav=isset($response[1])?$response[1]['Data'][2]['VarCharValue']:0;
                    // $value->nav_date=isset($response[1])?$response[1]['Data'][2]['VarCharValue']:NULL;
