    ...
    public function search(Request $request)
    {
        try {
            ...
            foreach ($all_data as $key => $value) {
                ...
                // if ($value->rnt_id==1) {
                //     $value->total_rows=DB::select('SELECT * FROM td_mutual_fund_trans WHERE folio_no="'.$value->folio_no.'" AND product_code="'.$value->product_code.'" AND trans_date<="'.$valuation_as_on.'"');
                // }else {
