    ...
    public function currAum1(Request $request)
    {
        try {
            ...
            if ($prev_date) {
                ...
                $rawQuery .= $condition_v ."(". $queryString . "=(SELECT MAX(trans_date) FROM td_mutual_fund_curr_aum WHERE trans_date <='" . $prev_date . "'))";
            }
            // return $rawQuery;
            // $startDates = [$date];
            // // return $startDates;
    ...
    public function currAumTrend(Request $request)
    {
        try {
            ...
            for ($i=1; $i <= 11; $i++) { 
                ...
                if ($valuation_as_on) {
                    $condition_v = (strlen($rawQuery) > 0) ? " OR " : " ";
                    $rawQuery .= $condition_v ."(". $queryString . "=(SELECT MAX(trans_date) FROM td_mutual_fund_curr_aum WHERE trans_date <='" . $valuation_as_on . "'))";
                }
